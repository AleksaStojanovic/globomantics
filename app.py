...
app.config["RECAPTCHA_PUBLIC_KEY"] = "6Lf5akQpAAAAAJ8yOLzHqTfWTsRNgXcNXAzqySMi"
app.config["RECAPTCHA_PRIVATE_KEY"] = "6Lf5akQpAAAAAGqb0I4JeB4JMtwQc5C6jal7gL9N"
...
class ItemForm(FlaskForm):
    title = StringField(
        ...
        validators=[
            ...
            Length(
                min=5, max=20, message="Input must be between 5 and 20 characters long"
            ),
        ],
    )
    price = PriceField(
    ...
    description = TextAreaField(
        ...
        validators=[
            ...
            DataRequired("Data is required!"),
            Length(
                min=5, max=40, message="Input must be between 5 and 40 characters long"
            ),
        ],
    )
    image = FileField(
...
class FilterForm(FlaskForm):
    ...
    price = SelectField(
        "Price", coerce=int, choices=[(0, "---"), (1, "Max to Min"), (2, "Min to Max")]
    )
    category = SelectField("Category", coerce=int)
...
def edit_item(item_id):
    ...
    if item:
        ...
        if form.validate_on_submit():
            ...
            flash(
                "Item {} has been successfully updated".format(form.title.data),
                "success",
            )
            return redirect(url_for("item", item_id=item_id))

        form.title.data = item["title"]
        form.description.data = Markup.unescape((item["description"]))
...
def item(item_id):
    ...
    if item:
        comments_from_db = c.execute(
            """select content from comments where item_id=? order by id desc""",
            (item_id,),
        )
...
def new_item():
    ...
    if form.validate_on_submit() and form.image.validate(
        ...
        flash(
            "Item {} has been successfully submitted".format(request.form.get("title")),
            "success",
        )
        return redirect(url_for("home"))
